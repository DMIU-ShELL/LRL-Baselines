...
'''
ppo, ewc (lifelong learning synaptic consolidation), task boundary (oracle) given
'''
def ppo_ll_mctgraph(name, args):
    ...
    config.env_config_path = env_config_path
    config.lr = 0.00015
    config.cl_preservation = 'ewc_multi_head' if args.multi_head is True else 'ewc_single_head'
    config.seed = args.seed
    random_seed(config.seed)
    exp_id = '-{0}-{1}'.format(config.seed, args.exp_id)
    #log_name = name + '-ppo' + '-' + config.cl_preservation + exp_id
...
if __name__ == '__main__':
    ...
    parser.add_argument('--multi_head', help='whether to use multi-head or not', default=False, action="store_true")
