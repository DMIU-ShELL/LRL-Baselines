...
def find_all_iteration_avg_reward_tags(event_file):
    ...
    iteration_avg_reward_tags = [tag for tag in tags if 'iteration_reward/avg' in tag]
    cl_eval_icr = [tag for tag in tags if 'cl_eval/icr' in tag]
    return iteration_avg_reward_tags, cl_eval_icr

def process_event_files(event_files, output_dir):
    for event_file in event_files:
        avg_tags, icr_tags = find_all_iteration_avg_reward_tags(event_file)
        if avg_tags:
            scalars_dict = extract_scalars_from_event_file(event_file, avg_tags)
            ...
            print(f"No iteration_reward/avg tags found in file {event_file}")

        '''if icr_tags:
            scalars_dict = extract_scalars_from_event_file(event_file, icr_tags)
            base_name = os.path.basename(event_file).split('.')[0]  # Remove extension from event file
            full_path = os.path.dirname(event_file)  # Get the full path of the event file
            save_scalars_to_csv(scalars_dict, output_dir, base_name, full_path)
        else:
            print(f"No cl_eval/icr tags found in file {event_file}")'''
