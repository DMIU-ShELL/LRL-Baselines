...
def ppo_ll_continualworld(name, args):
    ...
    exp_id = '-eval-run-{0}{1}'.format(config.seed, id_)
    del id_
    ...
    agent = load_agent(agent, model_path)
    ...
    for task_idx, task_data in enumerate(ret):
        ...
        if task_idx < new_task_idx:
            ...
            title = 'Task {0} (new task); EWC'.format(task_idx)
