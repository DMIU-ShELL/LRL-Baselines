...
if __name__ == '__main__':
    ...
    if args.env_name == 'continualworld':
        ...
        if args.algo == 'baseline':
            ...
            config.env_config_path = env_config_path
            config.lr = 5e-4
            config.cl_preservation = 'clear'
            config.seed = args.seed
            random_seed(config.seed)
            exp_id = '-{0}'.format(config.seed)
            log_name = name + '-ppo-clear' + exp_id
            config.log_dir = get_default_log_dir(log_name)
            ...
            # CLEAR hyperparams; for continuous we use action BC + value regression
            config.clear_buffer_size = int(2e5)
            config.clear_batch_size = 512
            config.clear_bc_weight = 1.0
            config.clear_v_weight = 1.0
            config.clear_replay_epochs = 1
            config.clear_num_mini_batches = 8

            agent = LLAgentCLEAR(config)
            config.agent_name = agent.__class__.__name__
            tasks = agent.config.cl_tasks_info
            config.cl_num_learn_blocks = 1
            shutil.copy(env_config_path, config.log_dir + '/env_config.json')
