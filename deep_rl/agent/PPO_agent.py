...
class PPOContinualLearnerAgent(BaseContinualLearnerAgent):
    def __init__(self, config):
        ...
        # Pre-create task embeddings so the optimizer tracks them from the start
        if hasattr(self.network, 'embeddings'):
            for t in tasks:
                tl = tensor(t['task_label'])
                self.network.embeddings.get(tl, trainable=False, freeze_others=False)
...
class LLAgentHNet(PPOContinualLearnerAgent):
    '''
    PPO continual learning agent using HyperNetworks (heads-only generation).
    Adds a regularizer to keep generated weights for past task codes close to stored snapshots.
    '''
    def __init__(self, config):
        PPOContinualLearnerAgent.__init__(self, config)
        self.curr_train_task_label = None
        # Store snapshots of generated heads per seen task label (as tensors)
        self._snapshots = []  # list of dicts with keys: 'label', 'weights', 'key'
        # Hyper-regularization
        self.hnet_reg_coeff = getattr(config, 'hnet_reg_coeff', 1e-3)
        self.hnet_reg_sample = getattr(config, 'hnet_reg_sample', 0)  # 0 = all

    def _generate_params(self, task_label, detach=True, trainable=True, freeze_others=True):
        tl = tensor(task_label).to(Config.DEVICE)
        if hasattr(self.network, 'generate_weights'):
            return self.network.generate_weights(tl, detach=detach, trainable=trainable, freeze_others=freeze_others)
        else:
            raise RuntimeError('Network does not implement generate_weights')

    def _penalty(self):
        if len(self._snapshots) == 0:
            return tensor(0.0)
        device = Config.DEVICE
        # Optionally subsample past tasks
        idxs = np.arange(len(self._snapshots))
        if self.hnet_reg_sample and self.hnet_reg_sample < len(idxs):
            np.random.shuffle(idxs)
            idxs = idxs[:self.hnet_reg_sample]
        loss = tensor(0.0).to(device)
        for i in idxs:
            entry = self._snapshots[i]
            tl = entry['label']
            snap = entry['weights']

            # Keep embeddings for past tasks frozen during the penalty term
            curr, _ = self._generate_params(tl, detach=False, trainable=False, freeze_others=False)

            for k in snap.keys():
                loss = loss + (curr[k].to(device) - snap[k].to(device)).pow(2).sum()
        return self.hnet_reg_coeff * loss

    def task_train_start(self, task_label):
        # Ensure the embedding for this task is trainable; freeze others
        if hasattr(self.network, 'embeddings'):
            tl = tensor(task_label).to(Config.DEVICE)
            self.network.embeddings.get(tl, trainable=True, freeze_others=True)
        self.curr_train_task_label = task_label
        return

    def task_train_end(self):
        # Snapshot current generated heads for current task label
        if self.curr_train_task_label is not None:
            tl = tensor(self.curr_train_task_label)
            weights, key = self._generate_params(tl, detach=True, trainable=False, freeze_others=False)
            self._snapshots.append({'label': tl.detach().cpu(),
                                    'weights': {k: v.detach().cpu() for k, v in weights.items()},
                                    'key': key})
            # Freeze the embedding for this finished task
            if hasattr(self.network, 'embeddings'):
                self.network.embeddings.freeze(key)
        self.curr_train_task_label = None
        return {'consolidate': (None, None), 'task_label_agent': self._snapshots[-1]['label'] if len(self._snapshots)>0 else None}
    ...
    def iteration(self):
        ...
        for _ in range(config.optimization_epochs):
            ...
            while not batcher.end():
                ...
                #batch_dim = sampled_states.shape[0]
                #batch_task_label_mb = torch.repeat_interleave(task_label.reshape(1, -1), batch_dim, dim=0)

                # Safe expansion for One-Hot or Integer labels
                batch_dim = sampled_states.shape[0]
                
                # Ensure task_label is a tensor and on device
                if not isinstance(task_label, torch.Tensor):
                    task_label = tensor(task_label)
                task_label = task_label.to(config.DEVICE)

                # If task_label is [1, 60] (One-Hot) -> [Batch, 60]
                # If task_label is [1] (Integer) -> [Batch, 1]
                if task_label.dim() == 1: 
                    task_label = task_label.unsqueeze(0) # Ensure [1, D]
                    
                batch_task_label_mb = task_label.expand(batch_dim, -1)

                _, _, log_probs, entropy_loss, values, outs = self.network.predict(sampled_states, \
                    sampled_actions, task_label=batch_task_label_mb, return_layer_output=True)
                ratio = (log_probs - sampled_log_probs_old).exp()
                obj = ratio * sampled_advantages
                obj_clipped = ratio.clamp(1.0 - self.config.ppo_ratio_clip,
                                          1.0 + self.config.ppo_ratio_clip) * sampled_advantages
                ...
                policy_loss_log.append(policy_loss.detach().cpu().numpy())
                value_loss_log.append(value_loss.detach().cpu().numpy())
                reg_log.append(reg_loss.detach().cpu().numpy())

                self.opt.zero_grad()
                (policy_loss + value_loss + reg_loss).backward()
                norm_ = nn.utils.clip_grad_norm_(self.network.parameters(), config.gradient_clip)
                grad_norm_log.append(norm_.detach().cpu().numpy())
                self.opt.step()
        ...
        return {'grad_norm': grad_norm_log, 'policy_loss': policy_loss_log, \
            'value_loss': value_loss_log, 'log_prob': log_probs_log, 'entropy': entropy_log, \
            'ppo_ratio': ratio_log, 'hnet_reg': reg_log}
