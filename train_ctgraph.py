...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_id', help='experiment id', default='ct8_md', type=str)
    ...
    if args.env_name == 'ctgraph':
        ...
        if args.algo == 'baseline':
            ...
            config.eval_task_fn = eval_task_fn
            config.optimizer_fn = lambda params, lr: torch.optim.RMSprop(params, lr=lr)
            config.network_fn = lambda state_dim, action_dim, label_dim: CategoricalActorCriticNet_FullHNet_CL(
                state_dim, action_dim, label_dim,
                hidden_units=(200, 200, 200),
                embed_dim=64, hyper_hidden=128)
            config.policy_fn = SamplePolicy
            config.state_normalizer = ImageNormalizer()
            config.discount = 0.99
            config.use_gae = True
            ...
            config.hnet_reg_coeff = 1e-3
            config.hnet_reg_sample = 0

            agent = LLAgentHNet(config)
            config.agent_name = agent.__class__.__name__
            tasks = agent.config.cl_tasks_info
            config.cl_num_learn_blocks = 1
            shutil.copy(env_config_path, config.log_dir + '/env_config.json')
