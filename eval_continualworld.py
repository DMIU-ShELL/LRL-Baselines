...
def run_episode_metaworld_te(agent, states, deterministic=False):
    epi_info = {'policy_output': [], 'sampled_action': [], 'log_prob': [], 'entropy': [],
        'value': [], 'agent_action': [], 'reward': [], 'terminal': [], 'success': [], 'state': []}
...
def _eval_autopilot(agent, tasks_info, autopilot_task_idx, new_task_optimal_path_states):
    ...
    for eval_task_idx, eval_task_info in enumerate(_tasks):
        ...
        if eval_task_idx != autopilot_task_idx:
            ...
            perf = []
            eps = []
            _, episode_info = run_episode_metaworld_te(agent, new_task_optimal_path_states)
...
def sac_ll_continualworld(name, args):
            ...
            for k, v in agent.network.named_parameters():
                if 'betas' in k:
                    ...
                    if len(k) == 3: # for network.fc_action.betas or network.fc_critic.betas
                        k = '.'.join(k[1:])
                    else: # for network.actor_body.layers.x.betas or network.critic_body.layers.x.betas
                        k = k[1] + '_' + k[2] + k[3] + '.' + k[4]
    ...
    for k, v in agent.network.named_parameters():
        ...
        if k_split[1] == 'actor_body': new_k = k_split[1] + '_' + k_split[2] + k_split[3]
        elif k_split[1] == 'critic_body': new_k = k_split[1] + '_' + k_split[2] + k_split[3]
        elif k_split[1] == 'fc_action': new_k = k_split[1]
        elif k_split[1] == 'fc_critic': new_k = k_split[1]
        elif k_split[1] == 'fc_log_std': new_k = k_split[1]
    ...
    config.logger.info(d['actor_body_layers0'].keys())
    #new_d = {}
    for k, v in d.items():
    ...
    for task_idx, task_data in enumerate(ret):
        ...
        for episode_idx, episode_data in enumerate(task_data):
            ...
            config.logger.info('task {0}, epsiode {1}, total reward: {2}, total success: {3}'\
                .format(task_idx, episode_idx, np.sum(episode_data['reward']), \
                np.sum(episode_data['success'])))
            action_buffer_sampled.append(episode_data['sampled_action'])
            action_buffer_final.append(episode_data['agent_action'])
        action_buffer_sampled = [[x.detach().cpu() for x in ep] for ep in action_buffer_sampled]
        #action_buffer_sampled = np.asarray(action_buffer_sampled, dtype=np.float32)
        #action_buffer_final = np.asarray(action_buffer_final, dtype=np.float32)
